...
<html>
<head>
    <title>Formulario PHP - Paso 2</title>
</head>
<body>
    <?php
    session_start();
    ...
    <?php
    echo "DEBUG: resta = $resta, acumulado = $acumulado, camionesDescargados = $camionesDescargados, camiones = $camiones, sacosEspeciales= $sacosEspeciales, sacosDefectuosos = $sacosDefectuosos, contenedorEspecial=$contenedorEspecial, contenedorNormal=$contenedorNormal, capacidadEspecial = $capacidadEspecial, capacidadNormal= $capacidadNormal ";

    if (isset($resta)) {
        if ($_SESSION["acumulado"] < $resta) {

            echo "<form method='post' action='formulario2.php'>";
            echo "<label for='psacos'>Peso del saco de cafe:</label>";
